"""
Fixed Code - Sistema de Gerenciamento de Biblioteca
Experimento 1: Debugging Multi-Tipo - Solução

Todos os 7 erros foram corrigidos.
"""
...
class Book:
    ...
    def return_book(self, user_id: str) -> bool:
        ...
        # CORRIGIDO: Incrementa ao devolver
        self.available_copies += 1
        ...
        for isbn, book in self.books.items():
            for user_id, due_date in book.borrowed_by.items():
                if datetime.now() > due_date:
                    # CORRIGIDO: Usa cálculo correto de multa
                    days_late = (datetime.now() - due_date).days
                    fine = days_late * 2.0  # R$ 2.00 por dia
